	...
	<main>
		...
		<h2>
			<table class="centered">
				<td>
					<ToolTip title="Type something that needs to get done, then press Enter or click ✔️ below to add it to the list.">
						<input name="inputField1" class="italic" type="text" bind:this={inputField} bind:value={input} placeholder="What needs to be done?" on:keypress={onKeyPress} size="25">
					</ToolTip>
				</td>
				<td>
					<ToolTip title="Click to add this new task to the list.">
						<button on:click={addTask(inputField.value)}><Emoji symbol="✔️" label="Add Task"></Emoji></button>
					</ToolTip>
				</td>
			</table>
		</h2>
						...
						<tr class:done={task.done}>
							<td>
								<ToolTip title="Click to toggle whether this task has been completed.">
									<button on:click={completeTask(task)}><Emoji symbol="🏆" label="Completed"></Emoji></button>
								</ToolTip>
							</td>
							<td>
							...
							<td>
								<ToolTip title="Click to delete this task from the list.">
									<button on:click={() => deleteTask(task)}><Emoji symbol="🗑️" label="Delete"></Emoji></button>
								</ToolTip>
							</td>
						</tr>
					{/each}
